...
def my_softmax(np_array):
    ...
    predsoft = np.exp(np_array - max_val) / np.sum(
        np.exp(np_array - max_val), axis=1, keepdims=True
    )
    return predsoft
...
def get_vp_from_sphere_coordinate_xY(sphere_point, sphere_centre, sphere_radius):
    z_coords = sphere_radius - np.sqrt(
        sphere_radius ** 2 - np.sum((sphere_point) ** 2, axis=1, keepdims=True)
    )
    sphere_point_3d = np.hstack((sphere_point, z_coords))

    y_coords = (
        (-sphere_radius / (sphere_point_3d[:, 2] - sphere_radius))
        * (sphere_point_3d[:, 1])
    ) + sphere_centre[1]
    x_coords = (
        (-sphere_radius / (sphere_point_3d[:, 2] - sphere_radius))
...
def get_vp_from_sphere_coordinate_xZ(sphere_point, sphere_centre, sphere_radius):
    ...
    sphere_point_3d = np.hstack(
        (sphere_point[:, 0], y_coords.squeeze(), sphere_point[:, 1])
    ).reshape(1, -1)

    y_coords = (
        (-sphere_radius / (sphere_point_3d[:, 2])) * (sphere_point_3d[:, 1])
    ) + sphere_centre[1]
    x_coords = (
        (-sphere_radius / (sphere_point_3d[:, 2])) * (sphere_point_3d[:, 0])
    ) + sphere_centre[0]
    return x_coords, y_coords
...
def get_horvpz_from_projected_4indices_modified(
    ...
    y_coord = -np.sqrt(
        all_sphere_radii[0] ** 2 - (req_coords[0] ** 2 + req_coords[1] ** 2)
    )
    input_points[0, :] = get_line_given_sphere_pointonspherenormaltoplane(
        all_sphere_centres[0, :], [req_coords[0], y_coord, req_coords[1]]
    )[:2]

    vpzx_xy_coords = np.array([req_coords[2], 0]).reshape(1, -1)
    input_points[1, 0] = get_vp_from_sphere_coordinate_xY(
        vpzx_xy_coords,
        sphere_centre=all_sphere_centres[2, :],
        sphere_radius=all_sphere_radii[2],
    )[0][0]

    vpzy_xZ_coords = np.array([0, req_coords[3]]).reshape(1, -1)
    input_points[1, 1] = get_vp_from_sphere_coordinate_xZ(
        vpzy_xZ_coords,
        sphere_centre=all_sphere_centres[3, :],
        sphere_radius=all_sphere_radii[3],
    )[1][0]
...
def get_intrinisic_extrinsic_params_from_horizonvector_vpz(
    ...
    horizon_translate_coordz = horizon_vectorform[2] + (
        (
            horizon_vectorform[0] * (re_width / 2)
            + horizon_vectorform[1] * (re_height / 2)
        )
    )
    ...
    roll_from_horizon = degrees(
        atan(-horizon_vectorform_center[0] / horizon_vectorform_center[1])
    )
    ...
    fx = np.sqrt(
        np.abs((scaled_vpz[0] - image_centre[0]) / horizon_vectorform_center[0])
    )
    if verbose:
        print("Stuff for fy")
        print((scaled_vpz[1] - image_centre[1]) / horizon_vectorform_center[1])
    fy = np.sqrt(
        np.abs((scaled_vpz[1] - image_centre[1]) / horizon_vectorform_center[1])
    )

    norm_vpz = np.sqrt(
        (scaled_vpz[0] - image_centre[0]) ** 2 + (scaled_vpz[1] - image_centre[1]) ** 2
    )
    my_tilt = 90 - degrees(
        atan(norm_vpz / fy)
    ...
    norm_hor = np.sqrt(
        (normal_to_hor_from_imcentre[0] - image_centre[0]) ** 2
...
def get_scaled_homography(H, target_height, estimated_xrange, estimated_yrange):
    ...
    (tw, th) = (
        int(np.round((estimated_xrange[1] - estimated_xrange[0]))),
        int(np.round((estimated_yrange[1] - estimated_yrange[0]))),
    )
...
def main(_):
    ...
    if FLAGS.model_name == "vgg-16":
        ...
        _G_MEAN = 116.78
        _B_MEAN = 103.94
        resnet_average_channels = np.array(
            np.concatenate(
                (
    ...
    if FLAGS.model_name == "vgg-16":
        ...
        my_img = (np.array(my_img, np.float32)) * (1.0 / 255)
        my_img = (my_img - 0.5) * 2
    else:
        print("Invalid CNN model name specified")
        return
    ...
    with tf.Graph().as_default():
        ...
        if FLAGS.model_name == "vgg-16":
            with slim.arg_scope(vgg.vgg_arg_scope()):
                logits, _ = vgg.vgg_16(
                    img, num_classes=num_bins * no_params_model, is_training=False
                )
        elif FLAGS.model_name == "inception-v4":
        ...
        with tf.Session() as sess:
            with slim.queues.QueueRunners(sess):
                ...
                for ln in range(no_params_model):
                    predsoft = my_softmax(
                        np_rawvals[i, :].reshape(no_params_model, -1)[ln, :][np.newaxis]
                    )
    ...
    plot_scaled_horizonvector_vpz_picture(
        img_cv,
    ...
    (
        ...
        est_range_u,
        est_range_v,
    ) = get_overhead_hmatrix_from_4cameraparams(
        fx=fx,
        fy=fy,
        my_tilt=my_tilt,
